...
def _default_command() -> list[str]:
    ...
    return [sys.executable, str(root / "scripts" / "gmail_monitor.py"), "--mode", "monitor"]


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Supervise gmail_monitor and exit on stale watchdog state so the service manager can restart it.")
        ...
        if time.time() >= startup_deadline:
            ...
            if healthy:
                ...
                if consecutive_failures >= failures_before_restart:
                    LOG.error("gmail-monitor unhealthy; terminating child so the service manager can restart it")
                    _terminate_child(child, force_after_seconds=terminate_grace_seconds)
                    return 1
