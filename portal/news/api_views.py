...
def update_article_link_validation(request):
    ...
    if "image_url" in data:
        article.image_url = str(data["image_url"]).strip()[:2000]
        update_fields.append("image_url")

    if "source_published_at" in data:
        parsed_source_published_at = _parse_datetime_utc(data.get("source_published_at"), default=None)
        if parsed_source_published_at is not None:
            article.source_published_at = parsed_source_published_at
            article.published_at = parsed_source_published_at
            update_fields.extend(["source_published_at", "published_at"])
