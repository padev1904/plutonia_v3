...
Runs as a daemon thread inside the gmail-monitor process,
exposing article review endpoints so that local services on the
same host can trigger decisions via HTTP instead of shelling
...
TELEGRAM_SINGLE_SPOC = os.getenv("TELEGRAM_SINGLE_SPOC", "false").strip().lower() in {"1", "true", "yes", "on"}
...
def _telegram_direct_send_allowed() -> bool:
    return not TELEGRAM_SINGLE_SPOC
